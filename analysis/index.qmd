...
# data for 2018
df2018 <- read_html(url2018) %>%
    html_node("table.wikitable") %>%
    html_table(fill = TRUE) %>%
    clean_names() %>%
    select(rank = art_museums,
        museum = art_museums_2,
        city = art_museums_3,
        visitors_2018 = art_museums_4) %>%
    filter(rank != "N.")
    
# data for 2020
df2020 <- read_html(url2020) %>%
    html_node("table.wikitable") %>%
    html_table(fill = TRUE) %>%
    clean_names() %>%
    select(rank = n,
        museum = art_museums_2,
        city = art_museums_3,
        visitors_2020 = visitors_annualy_a)

# data for 2020
df2022 <- read_html(url2022) %>%
    html_node("table.wikitable") %>%
    html_table(fill = TRUE) %>%
    clean_names() %>%
    select(rank, museum,
        city = country_and_city,
        visitors_2022 = visitors_annually)
